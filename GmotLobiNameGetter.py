...
def getTargetData():
    ...
    session = Session()
    gb_posts_result = ( session.query(GmotDbAccessor.GBPost.id, GmotDbAccessor.GBPost.lobi_name)
                        .limit(8000)
                        # .all()
                        )
    session.flush()
...
def updateRecordWithLobiName(gbPosts_dictList):
    ...
    session.bulk_update_mappings(GmotDbAccessor.GBPost, gb_post_mappings)
    session.flush()
